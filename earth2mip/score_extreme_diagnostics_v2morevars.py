...
from earth2mip.extreme_scoring_utils_v2morevars import (
    set_threshold,
...
def open_ifs(time, variables):
    if 'heat_index' in variables:
        assert 'heat_index' == variables, "'heat_index' must be scored on its own"
        assert time[:4] == '2023'
        ds = xarray.open_dataset(f"/pscratch/sd/a/amahesh/tigge/d2m_t2m_summer23/postprocessed/ifs_ens_forecast_2023-{time[5:7]}-{time[8:10]}_postprocessed_t2m_d2m_heat-index.nc")
        ds['step'] = ds['step'] + ds['time']
        return ds[['heat_index']].rename({'time' : 'initial_time'}).rename({'step' : 'time',
                                          'latitude' : 'lat',
                                          'longitude' : 'lon',
                                          'number' : 'ensemble'}).isel(initial_time=0, drop=True).set_index(time='time')
    if time[:4] == '2023':
        ...
        subset = ds[['2m_temperature', '10m_u_component_of_wind', '10m_v_component_of_wind']].sel(time=time)
        subset = subset.rename({'latitude' : 'lat',
                        'longitude' : 'lon',
                        'time' : 'initial_time',
                        'number' : 'ensemble'})

        subset['wind_speed10m'] = np.sqrt(subset['10m_u_component_of_wind'] ** 2 + \
                                         subset['10m_v_component_of_wind'] **2)
        subset = subset.assign_coords({'level' : ds['level'] * 100})
...
def open_operational_analysis(time, variables):
    if 'heat_index' in variables:
        ...
        t2m = ds['t2m']
        d2m = ds['d2m']
        rh = _calculate_rh_from_dewpoint(t2m, d2m)
        lookup_table = xarray.open_zarr("/pscratch/sd/a/amahesh/hens/prod_heat_index_lookup.zarr").load()
        print('loading heat index analysis')
        heat_index = lookup_table.sel(Rh=rh, Ta=t2m, method='nearest')
        print('loaded heat index analysis')
        return heat_index.reset_coords(['Rh', 'Ta'], drop=True)
    if time.year == 2023:
        ds = xarray.open_dataset("/pscratch/sd/a/amahesh/tigge/analysis/ifs_analysis-2023-{:02d}.grib".format(time.month))
        next_ds = xarray.open_dataset("/pscratch/sd/a/amahesh/tigge/analysis/ifs_analysis-2023-{:02d}.grib".format(time.month+1))
        ds = xarray.concat([ds, next_ds], dim='time')
        ...
        ds['wind_speed10m'] = np.sqrt(ds['10m_u_component_of_wind'] ** 2 + \
                                         ds['10m_v_component_of_wind'] **2)
        subset = ds[['2m_temperature', 'wind_speed10m']]
        subset = subset.rename({'latitude' : 'lat',
                        'longitude' : 'lon'})
...
def calculate_heat_index(t2m, d2m):
    lookup_table = xarray.open_zarr("/pscratch/sd/a/amahesh/hens/prod_heat_index_lookup.zarr").load()
    rh = _calculate_rh_from_dewpoint(t2m,
                                     d2m)
    return lookup_table.sel(Rh=rh, Ta=t2m, method='nearest')
...
def load_heat_index():
    ...
    # Base path excluding the timestamp part
    base_path = "/global/cfs/projectdirs/m1517/cascade/amahesh/hens/"

    # Initialize list to hold file paths
    file_paths, ds_list = [], []

    # Iterate through the date range, generating file paths
    current_date = start_date
    while current_date <= end_date:
        # Format date as YYYYMMDD for the path increment
        date_str = current_date.strftime("%Y%m%d")
        
        # Format the time as T000000 (which can be adjusted as needed, but assumed to be 00:00:00 for all files)
        time_str = "T000000"
        
        # Construct the dynamic part of the path (which includes the date and time)
        dynamic_part = f"HENS_summer23_{date_str}{time_str}/"

        date_str = current_date.strftime("%Y-%m-%d")
        
        # Construct the full file path by concatenating the base path with the dynamic part
        file_path = f"{base_path}{dynamic_part}ensemble_out_00000_{date_str}-00-00-00.nc"
        
        # Add the file path to the list
        file_paths.append(file_path)
        
        # Increment the date (move to the next day)
        current_date += timedelta(days=1)

        curr_ds = xarray.open_dataset(file_path, group='global')
        ds_list.append(curr_ds[['heat_index']].isel(time=0, ensemble=0, drop=True))

    import pandas as pd
    out = xarray.concat(ds_list, dim=xarray.DataArray(pd.date_range('2023-06-01', '2023-08-31', freq='D'),
                                         dims=['time']))
    return out
...
def open_verification(time):
    ...
    v['wind_speed10m'] = np.sqrt(v['u10m']**2 + v['v10m']**2)
    v = v.chunk({"time": 1})
    return v
...
def main(
    ...
    for domain in domains:
        ...
        if ifs:
            ds = open_ifs(input_path, variables)
        else:
        ...
        if score:
            ...
            if ifs:
                v = open_operational_analysis(date_obj, variables)
            else:
                if variables.split(",") == ['heat_index']:
                    print('loading heatindex')
                    v = load_heat_index()
                else:
                    v = open_verification(date_obj)
                    if date_obj.month == 12:
                        next_v = open_verification(date_obj+timedelta(days=31))
                        print('concatenating')
                        v = xarray.concat([v, next_v], dim='time')
            print(v)
            print(ds)
            shared = set(v) & set(ds)
            ...
            if precip:
                variables = ['tp']
            elif args.variables is not None:
                variables = args.variables.split(",")
            else:
                variables = ["t2m"] #, "wind_speed10m"]
            ds = ds[variables]
            verification = verification[variables]
            for variable in variables:
                print('Scoring variable ', variable)
                if variable == 't2m':
                    percentiles = [0.01] #0.95
                else:
                    percentiles = [0.99]
                for percentile in percentiles: #[0.95]: #['99p9']: # [0.99]: #, 0.99]:
                    greater_than = False if percentile == 0.05 else True
                    output_path_percentile = os.path.join(
                        output_path, f"percentile_{percentile}_{variable}/"
                    )
                    threshold = set_threshold(
                        variable, ds['time'], percentile, extreme_scoring,
                        ds, window=time_averaging_window
                    )
                    thresholded_ds = threshold_at_extremes(ds[[variable]], threshold, greater_than)
                    thresholded_ensemble_mean = thresholded_ds.mean("ensemble")
                    if greater_than:
                        thresholded_verification = xarray.where(verification[[variable]] > threshold, 1, 0)
                    else:
                        thresholded_verification = xarray.where(verification[[variable]] < threshold, 1, 0)
                    save_dataset(
                    ...
                    verification_var = verification[[variable]]
                    ds_var = ds[[variable]]
                    if greater_than:
                        twMAE = np.abs(
                            xarray.where(verification_var < threshold, threshold, verification_var) -
                            xarray.where(ds_var.isel(ensemble=0) < threshold, threshold, ds_var.isel(ensemble=0))
                        )
                        ...
                        twCRPS = xskillscore.crps_ensemble(
                            xarray.where(verification_var < threshold, threshold, verification_var),
                            xarray.where(ds_var < threshold, threshold, ds_var).chunk(
                                dict(ensemble=-1)
                            ),
                            issorted=False,
                            member_dim="ensemble",
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="string containing comma separated list of variables",
    )
