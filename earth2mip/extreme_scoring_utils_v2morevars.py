...
def set_threshold(variable, times, percentile, extreme_scoring, align_ds, window=""):
    """
    ...
    ------
         ...
         variable   : the variable for which to calculate the
                           threshold
    ...
    """
    if percentile != '99p9':
        percentile_str = "{:02d}".format((int(percentile * 100)))
    else:
        percentile_str = percentile
    assert variable in ['wind_speed10m', 't2m', 'heat_index'], "Extreme event validation is only implemented for t2m, heat_index, and  wind_speed10m,. These variables are included: {}".format(variable)
    if window == 'D':
        raise NotImplementedError("daily thresholds are not implemented")
        ...
        for curr_time in times:
            ...
            ds = xarray.open_zarr(
                "{}/{}_percentile{}_{:02d}_dailymean.zarr".format(
        ...
        thresholds = []
        if "05" in percentile_str or "01" in percentile_str:
            extreme_scoring = extreme_scoring.replace("thresholds", "thresholds_cold")
        for curr_time in times:
            curr_time_dt = earth2mip.time.convert_to_datetime(curr_time)
            ds = xarray.open_zarr(
                "{}/{}_percentile{}_{:02d}_{:02d}".format(
                    extreme_scoring,
                    variable,
                    percentile_str,
                    curr_time_dt.month,
        ...
        retval = xarray.concat(thresholds, dim=times).rename({'latitude' : 'lat', 'longitude' : 'lon'})
        print('loading threshold ', variable)
        print('loading threshold ' , "{}/{}_percentile{}_{:02d}_{:02d}".format(              
                    extreme_scoring,                                                            variable,
                    percentile_str,                                     
                    curr_time_dt.month,                                 
                    curr_time_dt.hour,                                  
                ) )
        if variable  == 't2m':
            retval = retval.rename({'VAR_2T' : 't2m'})
        retval = retval[[variable]]
        _, retval = xarray.align(align_ds, retval)
        return retval.chunk({'time': 1,
               'lat': 721,
               'lon': 1440})
...
def threshold_at_extremes(ds, threshold, greater_than):
    """
    inputs
    ------
           ...
           greater_than : bool whether to calculate greater than threshold or less than
    """
    if greater_than:
        print('binarizing greater than')
