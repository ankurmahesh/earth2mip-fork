...
def log_memory_usage(message=""):
    ...
    cpu_mem = process.memory_info().rss / 1024 ** 3  # Convert bytes to GB
    logger.info(f"{message} - CPU Memory Usage: {cpu_mem:.2f} GB")
    
    if torch.cuda.is_available():
        device = torch.device("cuda")
        cuda_mem_allocated = torch.cuda.memory_allocated(device) / 1024 ** 3  # Convert bytes to GB
        cuda_mem_reserved = torch.cuda.memory_reserved(device) / 1024 ** 3
        logger.info(f"{message} - CUDA Memory allocated: {cuda_mem_allocated:.2f} GB; Memory cached: {cuda_mem_reserved:.2f} GB")
    else:
...
async def lagged_average_simple(
    *,
    ...
    async for (j, k), ensemble, obs in core.yield_lagged_ensembles(
        ...
        log_memory_usage("After forecast")
